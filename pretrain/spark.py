...
class LaplaceFilter(object):
    def __init__(self, kernel_size=7, ddepth=cv2.CV_16S):
    ...
    def __call__(self, batch):
        transformed_batch = []
        for item in batch:
            # Apply your transformation logic to item
            transformed_item = self._apply_transform(item)
            transformed_batch.append(transformed_item)
        transformed_batch = torch.stack(transformed_batch)
        return transformed_batch.to(batch.device)

    def _apply_transform(self, tensor_img):
        ...
        laplace = cv2.normalize(
            laplace, None, 0, 1.0, cv2.NORM_MINMAX, dtype=cv2.CV_32F
        )
        laplace = np.transpose(laplace, (2, 0, 1))
        return torch.from_numpy(laplace)
...
class SparK(nn.Module):
    def __init__(
        ...
        self.laplace_filter = None
        if laplace_recon:
            self.laplace_filter = LaplaceFilter()
    ...
    def forward(self, inp_bchw: torch.Tensor, active_b1ff=None, vis=False):
        ...
        # Apply laplace filter to the input
        if self.laplace_filter is not None:
            inp_bchw = self.laplace_filter(inp_bchw)

        inp, rec = self.patchify(inp_bchw), self.patchify(
            rec_bchw
        )  # inp and rec: (B, L = f*f, N = C*downsample_raito**2)
        # Only apply normalization if laplace filter is not applied
        if self.laplace_filter is None:
